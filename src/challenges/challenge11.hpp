...
namespace euler {
class Challenge11 : public BaseChallenge<11> {
 public:
  bool run() const override;

 private:
  static int getMaxProduct(const std::vector<std::vector<int>>& grid,
                           const int i, const int j, const int consecutive);
  static int findLargestProduct(const std::vector<std::vector<int>>& grid,
                                const int consecutive);
};

int Challenge11::getMaxProduct(const std::vector<std::vector<int>>& grid,
                               const int i, const int j,
                               const int consecutive) {
  int maxProduct = 0;
  if (i - consecutive - 1 >= 0) {  // Free spaces down
    // Down
    auto tmp = grid[i][j];
    for (auto offset = 1; offset < consecutive; offset++)
      tmp *= grid[i - offset][j];
    maxProduct = std::max(maxProduct, tmp);
    if (j - consecutive - 1 >= 0) {  // Free spaces left
      // Down Left
      auto tmp = grid[i][j];
      for (auto offset = 1; offset < consecutive; offset++)
        tmp *= grid[i - offset][j - offset];
      maxProduct = std::max(maxProduct, tmp);
      // Left
      tmp = grid[i][j];
      for (auto offset = 1; offset < consecutive; offset++)
    ...
    if (j + consecutive - 1 < grid[0].size()) {  // Free spaces right
      // Down Right
      auto tmp = grid[i][j];
      for (auto offset = 1; offset < consecutive; offset++)
        tmp *= grid[i - offset][j + offset];
...
int Challenge11::findLargestProduct(const std::vector<std::vector<int>>& grid,
                                    const int consecutive) {
  int maxProduct = 0;
  for (int i = 0; i < grid.size(); i++)
    for (int j = 0; j < grid[0].size(); j++)
      maxProduct = std::max(maxProduct, getMaxProduct(grid, i, j, consecutive));
  return maxProduct;
}

bool Challenge11::run() const {
  ...
  int consecutive = 4;
  return findLargestProduct(grid, consecutive) == 70600674;
}
